...
public interface IngredientsStockRepository extends JpaRepository<IngredientsStockEntity, Long>{
     ...
     @Query(value="select a.ii_seq as seq , a.ii_name as name ,if(ifnull(c.is_stock,0)>=1,'판매 가능', '판매 불가') as stock from (select * from ingredients_stock b where b.is_si_seq = :store ) c right outer join ingredients_info a on a.ii_seq = c.is_ii_seq"
          ,nativeQuery = true)
