...
public interface DogInfoRepository extends JpaRepository<DogInfoEntity, Long>{
    ...
    //         +"from dog_info a "
    //         +"join (select * from dog_stock where dogs_si_seq=:store) b on a.dog_seq =b.dogs_dog_seq "
    //         +"join (select menu_dog_seq, count(menu_dog_seq) as 'count', min(menu_price) as 'price' from menu_info ) as  "
    //         +"where menu_bi_seq is null group by menu_dog_seq) c on c.menu_dog_seq = a.dog_seq "
    //         +"where a.dog_cate =:cate"
    //     , nativeQuery = true)
    @Query(value = "select if(c.count>1, a.dog_seq, c.menu_seq) as seq, a.dog_name as name , a.dog_detail as detail , a.dog_uri as uri , c.count, c.price, "
                    +"if(b.dogs_stock=0,'true', 'false') as soldout, 'SINGLE' as type "
            +"from dog_info a "
            +"join (select * from dog_stock where dogs_si_seq=:store) b on a.dog_seq =b.dogs_dog_seq "
            +"join (select menu_dog_seq, menu_seq , count(menu_dog_seq) as 'count', min(menu_price) as 'price' from menu_info "
            +"where menu_bi_seq is null group by menu_dog_seq) c on c.menu_dog_seq = a.dog_seq "
            +"where a.dog_cate =:cate"
        , nativeQuery = true)
